...
abstract class TemplateParams extends Object
{
	...
	{
		return $this->get_param($name, false);
	}
	 ...
	 * Возвращает параметр
	 * @param $name String: Имя параметра
	 * @param $throw_error Boolean: Определяет, вызывать ли исключение вместо возврата false
	 * @retval TemplateParam Параметр
	 */
	public function get_param($name, $throw_error = true)
	{
		$this->load();
	 ...
	 */
	public function get_params($params, $associative = true, $throw_error = true)
	{
		$result = array();

		while (list(, $name) = each($params))
		{
			$param = $this->get_param($name, $throw_error);
			if ($param === false)
				return false;
...
class ObjectTemplates extends TemplateParams
{
	...
	public function set_page($page_name)
	{
		...
		$content_type = $template->get_param("Pages::Content-type", false);
