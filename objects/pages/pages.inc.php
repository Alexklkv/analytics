...
class ObjectPages extends Object implements DatabaseInterface
{
	...
	static public function get_queries()
	{
		return array(
			...
			'search'		=> "SELECT * FROM @ppages WHERE content LIKE @s OR params LIKE @s ORDER BY name ASC"
		);
	}
	...
	public function search($text)
	{
		...
		$element_template = $this->Templates->get("/Панель администрирования/Страницы/Шаблоны/Элемент списка поиска");

		$text = "%{$text}%";
		$result = $this->DB->search($text, $text);
		while ($row = $result->fetch())
		{
			$element_template->bind_params($row);
			$output .= (string) $element_template;
		}
