...
abstract class Layer
{
	...
	public function &get($name)
	{
		...
		trigger_error("Can't find object {$name}", E_USER_ERROR);
	}
	...
	private function parse(&$content, $parsable)
	{
		...
		for ($i = 0; $i < $len; $i++)
		{
			switch ($content[$i])
			{
				...
				{
					if ($i == $len - 1)
						trigger_error("Unexpected open bracket at {$i}", E_USER_ERROR);
				...
				case '}':
				{
					if ($last_bracket_type === null)
						trigger_error("Unexpected close bracket", E_USER_ERROR);
					...
					if ($open['type'] != $close['type'])
						trigger_error("No open bracket for close bracket with type {$close['type']}", E_USER_ERROR);
		...
		if (!empty($open_brackets))
			trigger_error("Open/close brackets mismatch", E_USER_ERROR);
