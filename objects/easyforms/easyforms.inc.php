...
class ObjectEasyForms extends Object
{
	 ...
	 * @param $exceptions Array: Поля с альтернативным экранированием символов.
	 */
	public function bind($values, $exceptions = array())
	{
		reset($values);
		while (list($name, $value) = each($values))
		{
			$flags = isset($exceptions[$name]) ? $exceptions[$name] : ENT_COMPAT;
	...
	private function get_array($name, $params, &$values)
	{
		...
		if ($params['noempty'] && empty($array))
		{
			$this->add_error($name, $params);
			return false;
		}
	...
	private function add_error($name, $params)
	{
		if (isset($params['id']))
			$name = $params['id'];
