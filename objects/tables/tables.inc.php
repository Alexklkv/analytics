	...
	private function send_result($data, $result, $table)
	{
		...
		if ($template !== false)
		{
			...
			for ($i = 0; $i < $this->columns_count; $i++)
				$templates[$i] = $template->get_param("col_".($i + 1), false);
		}
		...
		while (($row = $result->fetch()))
		{
			...
			if (!empty($table['format']))
				call_user_func_array($table['format'], array(&$row));

			reset($table['fields']);
			while ((list($index, $field) = each($table['fields'])))
				$fields[$index] = $row[$field];

			if ($template !== false)
				$this->Common->apply_templates($fields, $templates, $row);
