...
class Database
{
	...
	public function multi_query($queries, $params)
	{
		...
		if (!$this->sql->multi_query($query))
			trigger_error("Error occured while executing multi-query: ".$query, E_USER_ERROR);
	...
	private function get_name($query, $cur_pos)
	{
		...
		trigger_error("Named placeholder parse error", E_USER_ERROR);
	}
		...
		while (true)
		{
			...
			switch ($type)
			{
				...
				case 'A':	// AND
					...
					$replacement .= $this->make_pairs($param, " AND ");
					break;
				case '@':
					...
					break;
				default:
					trigger_error("Unknown placeholder @{$type}", E_USER_ERROR);
			}
	...
	private function connect()
	{
		...
		if ($sql->connect_error)
			trigger_error("Can't connect to DB", E_USER_ERROR);

		if ($sql->query("SET NAMES utf8") === false)
			trigger_error("Can't set encoding", E_USER_ERROR);
	...
	private function execute($query)
	{
		...
		if ($result === false)
			trigger_error("Error occured while executing query: ".$query.", error: ".$this->sql->error.", errno: ".$this->sql->errno, E_USER_ERROR);
	...
	private function next_param(&$params, $is_array = false, $required = true, $protect = true)
	{
		...
		if ($param === false)
		{
			if (!$required)
				return false;
			trigger_error("No param for placeholder", E_USER_ERROR);
		}
		...
		else
		{
			if (is_array($param) !== $is_array)
				trigger_error("Expected param type mismatch", E_USER_ERROR);
		}
...
class DatabaseResult
{
	...
	public function fetch_all($field = false)
	{
		$result = array();
		while (($row = $this->fetch($field)))
			$result[] = $row;
...
class DatabaseConnector
{
	...
	public function __call($query_name, $params)
	{
		if (!isset($this->queries[$query_name]))
			trigger_error("Unknown query name {$query_name}", E_USER_ERROR);
		...
		if (is_array($query))
		{
			...
			if ($count == 0)
				trigger_error("Can't run empty query", E_USER_ERROR);
			if ($count != 1)
