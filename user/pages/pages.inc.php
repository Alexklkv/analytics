...
class AdminPages extends ComponentAdmin
{
	...
	public function on_index()
	{
		...
		$name = $this->EasyForms->field("name", INPUT_GET);
		if ($name === false)
			return;

		if (!preg_match('//u', $name))
			$name = iconv("WINDOWS-1251", "UTF-8", $name);
		$this->Templates->bind_params(array('page' => $name));
	}

	/**
	 * Поиск страниц с заданным контентом
	 */
	public function on_search()
	{
		$this->Templates->set_template("Панель администрирования/Шаблоны/Страница");
		$this->Templates->set_page("Поиск");
		$search_text = $this->EasyForms->field("search_text", INPUT_GET);

		if ($search_text === false)
			return;

		$search_result = $this->Pages->search($search_text);
		$params = array('results' => $search_result, 'search_text' => $search_text);
		$this->Templates->bind_params($params);
	}
	...
	public function on_restore()
	{
		...
		$fields['time']		= $_SERVER['REQUEST_TIME'];
	...
	public function on_versions()
	{
		...
		if (!empty($fields['user_id']))
		{
			...
			if ($user !== false)
				$this->Templates->email = $user['email'];
			else
		...
		list($version, $date) = $this->Templates->get_params(array("version", "date"), false);
	...
	public function on_add_submit()
	{
		...
		$fields['updated'] = $_SERVER['REQUEST_TIME'];
	...
	public function on_delete()
	{
		...
		$item = $this->Templates->get_param("item");
	...
	public function on_get_menu()
	{
		...
		$structure = $this->gen_menu($fields['prefix'], $fields['filter']);
