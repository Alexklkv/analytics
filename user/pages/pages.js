...
function pages_resize()
{
	...
	var body_height = $("body").height() - 20;
	var head_height = $(".content").offset().top;
	var container_height = $(".content_container").height();

	var tabs_width = $("#tabs").width();

	var content_height = body_height - (head_height + container_height);
	if (content_height < 0)
		content_height = 100;
	...
	var params_height = body_height - (head_height + container_height);
	if (params_height < 0)
		params_height = 0;
...
function pages_content_init()
{
	...
	{
		'create': pages_resize,
		'activate': pages_resize
	});

	TabKeys.init("#content");
	TabKeys.init("#params textarea");
	...
	$("#name").bind("keyup", function()
	{
		if (this.value != $("input[name='orig_name']").val())
			content_apply.prop("disabled", true);
		else
			content_apply.prop("disabled", false);
	});
...
function pages_params_init(param_names, param_contents)
{
	$(document).ready(function()
	{
		...
		if (!pages_get_readonly())
		{
			...
			$("#params .ui-tabs-nav").bind("dblclick", pages_params_add);
			$(document).on("click", "#params span.ui-icon-close", pages_params_delete);
		}
		else
		{
			$("#params").tabs(
...
function pages_params_delete(event)
{
	event.stopImmediatePropagation();

	if (!confirm("Вы действительно хотите удалить этот параметр?"))
		return false;
	...
	var tabs = params.find(".ui-tabs-nav li");

	var index = tabs.index($(this).parent());

	var tab = tabs.eq(index).remove();
	var panelId = tab.attr("aria-controls");
	$("#" + panelId).remove();
	params.tabs("refresh");

	var left = params.find(".ui-tabs-nav li").length;
	if (left != 0)
		return false;
...
function pages_params_update(param, param_id)
{
	...
	$("#params .ui-tabs-active a").text(param_name);
}
...
function pages_params_add(param_name)
{
	...
	params.find(".ui-tabs-nav").append("<li><a href='#params-" + param_id + "'>" + param_name + "</a> <span class='ui-icon ui-icon-close'>X</span></li>");

	var param_template = $("#param_template").html().replace(new RegExp("{param_id}", "g"), param_id);
	params.append("<div id='params-"+ param_id +"'>" + param_template + "</div>");
	...
	var last = params.find(".ui-tabs-nav li").length - 1;
	params.tabs("option", "active", last);
...
Loader.scripts(["ckeditor"], "pages_editor_init");
...
function pages_editor_init()
{
	...
	$(document).on("click", ".pages_editor_create", pages_editor_create);
...
function pages_versions_init(user_id, session_id, final_value)
{
	...
	$(document).on("click", "#path a", function()
	{
...
{
	$(document).on("click", "#check_all", Inputs.checkbox_toogle);
}
