...
class ManagerAdmin extends Component
{
	...
	{
		$module = ComponentAdmin::get_module();

		if (!isset($this->accesses[$module]))
			$this->Log->error("Can't check module {$module} access");
	...
	private function load_modules()
	{
		...
		while (list($module) = each($modules))
		{
			...
			$this->modules[$module] = $class->get_name();
			$this->services[$module] = $class->get_services();
	...
	private function bind_modules()
	{
		$active_module = ComponentAdmin::get_module();
		$active_action = ComponentAdmin::get_action();

		$template = $this->Templates->get("/Панель администрирования/Шаблоны/Модуль");

		list($link, $dropdown) = $template->get_params(array("link", "dropdown"), false);
		...
		{
			$accesses = $this->accesses[$module];
			$services = $this->services[$module];

			if (empty($accesses) || empty($services))
				continue;
			...
			reset($services);
			while ((list($action, $title) = each($services)))
			{
				if ($accesses[$action] && !$this->Admin->check_access($module."_".$action))
					continue;

				$active = false;
				if ($module == $active_module && $action == $active_action)
				{
					$active = true;
					$active_title = $title;
				}

				$link->bind_params(array('module' => $module, 'action' => $action, 'title' => $title, 'active' => $active));
			...
			if (count($services) != 1)
			{
				$dropdown->name = $this->modules[$module];
				$dropdown->links = $links;
				$dropdown->title = $active_title;

				$links = (string) $dropdown;
			}

			$modules .= $links;
		}

		$this->Templates->modules = $modules;
	}

	private function get_list()
	{
		$list = explode(",", ENGINE_MODULES);
		...
		while ((list(, $module) = each($list)))
		{
			...
			if (!file_exists($module."/".$module.".inc.php"))
				$this->Log->error("Can't find module {$module} file");
	...
	private function login()
	{
		...
		$this->Admin->set_email($fields['login']);
